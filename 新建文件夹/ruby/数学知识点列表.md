...
111. 有相同正负惯性指数 **合同**. 正定 $x\ne0,f(x)>0$  
## 统计
112. $P(A\cup B)=P(A)+P(B)-P(AB), P(A-B)=P(A)-P(AB),P(A|B)=\frac{P(AB)}{P(B)}$
...
114. 常见的分布
    ...
    + 超几何分布（不放回抽取）
    + 均匀分布 
115. 分布函数 $P\{X\le a\}=F(a), P\{X=a\}=F(a)-F(a^-),F(a)=\int_{-\infty}^a f(x)dx,P\{a<x<b\}$. 注意 连续型点概率为零.
...
130. 区间估计 置信区间 $P\{|\frac{\overline{X}-\mu}{\sigma/\sqrt{n}}|<z_{\frac{\alpha}{2}}\}=1-\alpha$ 得到 $\mu$ 以 $\overline{X}$ 为中心置信度为 $1-\alpha$ 的区间
    + 假设检验 $H_0:\mu=\mu_0,P(H_1|H_0)=\alpha$，$\overline{X}$ 在含 $\mu_0$ 的区间外则拒绝 $H_0$，$\alpha$ 为显著性水平
