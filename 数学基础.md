...
- 行列式按行列*展开* *代数*余子式 例 $\begin{bmatrix} a&b \\ c&d\end{bmatrix}=ad-bc$ 符号 $(-1)^{i+j}$ 
- 对角/三角 分块矩阵 $(-1)^{MN}$
...
+ $ \sum X_i^2 \sim \chi^2(n),E=n,D=$  t分布 F 分布
...
2. $|a+b|<|a|+|b|, |x|+\frac{1}{|x|}>=2,\frac{1}{n+n}<\frac{1}{n+1}<\frac{1}{n}$ 
3. 常见不等式 当 $x>0$ 时  $e^x>1+x,ln(1+x)<x,\sin x<x$ 复合函数单调性
4. 欧拉公式 $e^{i\theta}=\cos\theta+i\sin\theta$ 
5. sum 1/(n ln n) 发散，证明
6. 数学归纳法，递推关系
7. 周期性，柯西不等式,
8. 级数求导可能会少一项常数项
