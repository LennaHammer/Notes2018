...
- 幂函数的极限 对数法 例 $\lim\limits_{x\to+\infty} \sqrt[x]{x}=e^{\lim\limits_{x\to+\infty}\frac{\ln x}{x}}=1,\lim\limits_{x\to0}x^x=e^{\lim\limits_{x\to0}x\ln x}=1$ 
    -  $1^\infty$型 例 $\lim\limits_{x\to0}(1+x)^{\frac{1}{x}}=e$ ，$\lim\limits_{v\to 0}(1+v)^u=e^{u\ln(1+v)}=e^{uv}$ 
- 注意 $\lim\limits_{x\to 0}e^\frac{1}{x}$，分段函数 $e^x,\arctan x,\frac{1}{x},\frac{ax+b}{cx+d}$ 分左右极限
...
- 隐函数的偏导数，等式两边求导求微分。对 $x$ 求导 $z=z(x,y)\Rightarrow x'=1,y'=0,z'=z_x'$. 
...
+ 常见积分 $\int x^n\mathrm{d}x=\frac{x^{n+1}}{n+1} + C,\int\frac{1}{x}\mathrm{d}x=\ln|x|+C$ 
  ...
  + 例 $\int\frac{1}{x^2}dx=-\frac{1}{x}+C,\int\frac{1}{\sqrt x}=2\sqrt{x}+C,\int\sqrt{x}dx=$  

  - $\int e^x dx=e^x+C,\int \sin x dx = -\cos x,\int \cos dx = \sin x$ 微分的逆运算

  - 例 $\int\frac{1}{1+x^2}dx=\arctan x+C,\int\sec^2 x dx = \tan x + C$  

- 第一类换元 $t=f(x)$ 例 $\int e^{-2x+1}dx=-\frac{1}{2}\int e^{-2x+1}d(-2x+1)$ ，
...
- $\iint f(x,y,z)\mathrm{d}S$ 其中 $\mathrm dS=\sqrt{1+z_x'^2+z_y'^2}dxdy$
